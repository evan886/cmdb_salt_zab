...
def loginZabbix():
    '''
    登录zabbix获取session
    https://blog.csdn.net/qq_37534835/article/details/89156591
    '''
    # url and url header
    # zabbix的api 地址，用户名，密码，这里修改为自己实际的参数
    zabbix_url = "http://z.cn/api_jsonrpc.php"
    # zabbix_header = {"Content-Type": "application/json"}
    zabbix_user = "Admin"
    zabbix_pass = "zabbix"

    # 用户认证信息的部分，最终得到一个sessionID 平时说的token
    ...
    auth_data = json.dumps({
        ...
        "id": 0 # #  1 是 其它什么 添加  groupid by evan by evan
...
evan@myxps:~$ python3 tmp/dapi.py  #id 0
['http://z.cn/api_jsonrpc.php', '4e33babbdebde4db789e0fe12717f0a6']
evan@myxps:~$ 
...
def getMonitor(request):
    ...
    '''获取提交的主机ID  server http://z.cn/hosts.php?form=update&hostid=10084 is 10084'''
    try:
        if request.method == "POST":
...
def dataHandle(*args,**kw):
    ...
    return result['result']  # json to 字典
'''
